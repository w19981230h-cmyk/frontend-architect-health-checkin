...
export function validateImport(rows,org,people,existing=[]){
 ...
 body.forEach((cells,index)=>{const values=Object.fromEntries(headers.map((h,i)=>[h,(cells[i]||'').trim()]));const name=values['科室名称'],type=values['科室类型'],state=values['科室状态']||'启用';const members=[...new Set((values['科室成员']||'').split(/[;；、]/).map(n=>n.trim()).filter(Boolean))],owner=values['管理员']||'';const problems=[];
 if(!name||name.length>100)problems.push('科室名称必填且不超过100字');if(names.has(name))problems.push('科室名称重复');names.add(name);
 if(!['门诊科室','住院科室'].includes(type))problems.push('科室类型须为门诊科室或住院科室');if(!['启用','停用'].includes(state))problems.push('科室状态须为启用或停用');if(values['所属机构']&&values['所属机构']!==org)problems.push('所属机构须为当前机构');
...
export function templateCsv(org){return '\uFEFF'+[importHeaders,['示例科室（请修改）','填写科室简介',org,'门诊科室','启用','填写科室备注','','']].map(r=>r.map(v=>'"'+v.replace(/"/g,'""')+'"').join(',')).join('\r\n');}
