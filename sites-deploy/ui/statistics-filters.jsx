...
export function mount(host,onApply,engine,initialValues={},config={}){
 const controller={};const hierarchy=!!engine.hierarchical;
 const defaults={...initial,...initialValues};
 function Filters(){
  const [values,setValues]=useState(defaults);
  controller.setValues=setValues;
  ...
  const options=(label,items)=>[{value:'',label},...items.map(x=>({value:x,label:x==='CKD'?'慢性肾病（CKD）':hierarchy?x.split('·').at(-1):x}))];
  const departments=values.org?(engine.orgs[values.org]||[]):[...new Set(Object.values(engine.orgs).flat())];
  const people=[...new Set(engine.people.filter(p=>(!values.org||p.org===values.org)&&(!values.dept||p.dept===values.dept)&&(!hierarchy||!values.team||p.team===values.team)).map(p=>p.name))];
  const teams=hierarchy?['全部团队',...new Set(engine.people.filter(p=>p.org===values.org&&p.dept===values.dept).map(p=>p.team))]:config.teams||['全部团队','心内科随访团队','肾病管理团队','糖尿病共管团队'];
  ...
  return <ConfigProvider locale={zhCN} theme={{token:{colorPrimary:'#1677ff',borderRadius:6,controlHeight:40,fontSize:14}}}>
   <Form layout="inline" colon={false} className={`sr-ant-filters${isOperations?' os-ant-filters':''}`} aria-label={isOperations?'统计报表查询':'数据报表查询'} onFinish={()=>onApply(values)}>
    ...
    <Form.Item label={isOperations?'机构':'集团'}><Select aria-label={isOperations?'机构':'集团'} value={values.org} options={options(config.orgLabel||(isOperations?'本院':'全部集团'),Object.keys(engine.orgs))} onChange={v=>change('org',v)} style={{width:'100%'}} /></Form.Item>
    <Form.Item label="科室"><Select aria-label="科室" disabled={hierarchy&&!values.org} value={values.dept} options={options('全部科室',departments)} onChange={v=>change('dept',v)} style={{width:144}} /></Form.Item>
    {isOperations&&<Form.Item label="团队"><Select aria-label="团队" disabled={hierarchy&&!values.dept} value={values.team} options={teams.map((x,index)=>({value:index?x:'',label:hierarchy?x.split('·').at(-1):x}))} onChange={v=>change('team',v)} style={{width:'100%'}} /></Form.Item>}
    <Form.Item label="个人"><Select aria-label="个人" disabled={hierarchy&&!values.team} value={values.person} options={options('全部人员',people)} onChange={v=>change('person',v)} style={{width:'100%'}} /></Form.Item>
