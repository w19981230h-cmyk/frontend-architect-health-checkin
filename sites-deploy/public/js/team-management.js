(function () {
  ...
  function showPlanDialog(type, plan = null, version = null) {
    ...
    if (type === 'delete' && plan) {
      ...
      const sortedVersions = [...plan.versions].sort((left, right) => {
        const leftEnabled = left.number === plan.enabledVersion;
        const rightEnabled = right.number === plan.enabledVersion;
  ...
  document.addEventListener('click', event => {
    ...
    if (togglePlan) {
      ...
      if (plan.enabledVersion != null) {
        ...
        if (!version) return;
        if (!version.published) { window.showToast?.('待发布版本不可启用，请先发布'); return; }
