...
(() => {
  ...
  root.innerHTML=`<div class="dl-dashboard dv-dashboard">
    <header class="dl-header"><h1>AI患者运营管理驾驶舱</h1><span class="dl-subtitle">区域总览 · 人群覆盖 · 机构分布</span><span class="dl-badge">模拟数据</span><span>统计截至 ${d.end}</span></header>
    ...
    <div class="dv-horizontal-overview"><p class="dv-overview-caption">本期筛查人群</p><div class="dv-horizontal-stages">${[
      ['总体人数',t.due,'未筛 '+f(t.due-t.screened)+' 人'],
      ['已筛查',t.screened,'未见异常 '+f(t.screened-t.abnormal)+' 人'],
      ['筛查异常',t.abnormal,'不符合入组 '+f(d.ineligible)+' 人'],
      ['符合入组',t.eligible,'未入组 '+f(t.eligible-t.enrolled)+' · 其中超期 '+d.overdueEnrollment],
      ['已入组',t.enrolled,'该批期末仍在管 '+f(d.cohortActive)+' 人']
    ].map(([label,value,desc],i)=>`<div class="dv-horizontal-stage ${i===4?'is-enrolled':''}"><div><span>${label}</span><strong>${f(value)}<small>人</small></strong></div><p>${desc}</p></div>`).join('')}</div><div class="dv-horizontal-rates">筛查覆盖 ${rate(t.screened,t.due)}<i>·</i>异常率 ${rate(t.abnormal,t.screened)}<i>·</i>入组适配率 ${rate(t.eligible,t.abnormal)}<i>·</i>入组转化 ${rate(t.enrolled,t.eligible)}</div><div class="dv-period-management"><p>全部来源 · 期末管理</p><div class="dv-period-body"><div class="dv-period-counts"><span>应管理 <b>${f(t.managedDue)} 人</b></span><span>当前在管 <b>${f(t.active)} 人</b></span><span>未在管 <b>${f(t.managedDue-t.active)} 人</b></span></div><div class="dv-period-coverage"><div>管理覆盖率 <strong>${rate(t.active,t.managedDue)}</strong></div><div class="dv-chart" data-compact="true" data-color="#21aa93" data-value="${percent(t.active,t.managedDue)}" role="img" aria-label="管理覆盖率 ${rate(t.active,t.managedDue)}"></div></div></div><small>筛查流程跟随同一批人；期末管理包含历史入组与其他来源。</small></div></div>
    </section>
    <section class="dl-card dv-levels dv-levels-compact"><div class="dv-levels-heading">${title(2,'三色管理')}<span>全部 ${d.organizations.length} 家机构 · 模拟数据</span></div><div class="dv-levels-strip"><div class="dv-levels-population"><h3>当前应管理人群 · 三色覆盖</h3><div class="dv-levels-population-body"><div id="dlPie" class="dl-pie" role="img" aria-label="应管理人群 ${f(t.managedDue)} 人的等级分布"></div><div><strong>应管理 ${f(t.managedDue)} 人</strong><p>红 ${f(d.levels[0].due)} / 黄 ${f(d.levels[1].due)}</p><p>绿 ${f(d.levels[2].due)} / 未分级 ${f(d.levels[3].due)}</p></div></div></div>${d.levels.slice(0,3).map((r,i)=>`<article class="dv-level-mini"><header><span style="color:${['#eb6371','#b69931','#4dac9c'][i]}">${r.name}管理人群</span><span>应管 ${f(r.due)} 人</span></header><div class="dv-level-mini-counts"><span>在管 <b>${f(r.active)}</b></span><span>未在管 <b>${f(r.due-r.active)}</b></span></div><div class="dv-level-mini-rates"><span>覆盖 ${rate(r.active,r.due)}</span><span>规范 ${r.standard}/${r.evaluable} · ${rate(r.standard,r.evaluable)}</span></div><div class="dv-chart dv-level-coverage" data-compact="true" data-value="${percent(r.active,r.due)}" role="img" aria-label="${r.name}管理覆盖率${rate(r.active,r.due)}；规范管理率${rate(r.standard,r.evaluable)}"></div></article>`).join('')}</div>${institutionDistribution}</section>
    <section class="dl-card dv-outcomes">${title(3,'健康管理成效','按病种独立评价')}<div class="dv-eval"><span>${icon('clipboard')}到期应评价 <b>${f(d.evaluationDue)}</b> 例</span><span>${icon('check')}可评价 <b>${f(sum(d.outcomes,'evaluable'))}</b> 例</span><span>${icon('clock','#ff7900')}待评价 <b>${d.evaluationDue-sum(d.outcomes,'evaluable')}</b> 例</span></div>${table(['病种','成效指标','可评价病例（例）','达到目标病例（例）','达标或稳定比例'],d.outcomes.map(r=>[r.name,r.metric,r.evaluable,r.achieved,bar(r.achieved,r.evaluable)]))}${note('评价目标与观察窗口按院内病种规则确定；不同病种结果不直接平均。以上为模拟数据。')}</section>
    <section class="dl-card dv-safety">${title(4,'风险与安全')}<div class="dv-safety-stats">${[['alert','高危预警患者',sum(d.risks,'high'),'人','#ff506b'],['bell','未处置预警',sum(d.risks,'unhandled'),'次','#ff7900'],['clock','其中超时未处置',sum(d.risks,'overdue'),'次','#ff506b'],['shield','及时处置率',d.riskTotals.timely.toFixed(1)+'%','','#168cff'],['link','预警闭环率',d.riskTotals.closed.toFixed(1)+'%','','#14c8ad']].map(a=>`<div>${icon(a[0],a[4])}<span>${a[1]}<b>${a[2]} <small>${a[3]}</small></b></span></div>`).join('')}<div>已处置未闭环 <b>${d.riskTotals.handledOpen}</b> 次</div></div>${table(['机构','高危患者（人）','未处置（次）','其中超时（次）','及时处置率','预警闭环率'],d.risks.map(r=>[r.name,r.high,`<span class="dl-red">${r.unhandled}</span>`,`<span class="dl-red">${r.overdue}</span>`,bar(r.timely,100),bar(r.closed,100)]))}${note(`预警共 ${d.riskTotals.events} 次 = 已闭环 ${d.riskTotals.closedCount} + 已处置未闭环 ${d.riskTotals.handledOpen} + 未处置 ${d.riskTotals.unhandled}；到期 ${d.riskTotals.due} 次，及时处置 ${d.riskTotals.timelyCount} 次。两项比率均以到期事件为分母；高危人数按人去重。`)}</section>
    <section class="dl-card dv-referrals">${title(5,'双向转诊')}<div class="dv-flows">${flow(false)}${flow(true)}</div>${table(['机构','上转申请（单）','上转未闭环（单）','上转闭环率','下转申请（单）','下转未闭环（单）','下转闭环率'],[referralTotal,...d.referrals].map(referralRow))}${note('按本期申请单跟踪；完成诊疗反馈或首次服务并确认结果后闭环。')}</section>
    <footer class="dl-footer">模拟数据 · 按患者、病例、周期、预警及转诊明细统一汇总 · 不含真实患者信息</footer>
  ...
  root.querySelectorAll('[data-org]').forEach(btn=>btn.onclick=()=>{const r=d.organizations.find(r=>r.name===btn.dataset.org);document.getElementById('dlDialogText').textContent=`${r.name}：总体人数 ${f(r.due)} 人，已筛 ${f(r.screened)} 人，异常 ${r.abnormal} 人，符合入组 ${r.eligible} 人，已入组 ${r.enrolled} 人。全部来源应管理 ${f(r.managedDue)} 人，在管 ${f(r.active)} 人，未在管 ${r.managedDue-r.active} 人。未提供科室级数据，暂不展示下级明细。`;dialog.showModal();});
  StatisticsFilters.mount(document.getElementById('dlFilters'),v=>{const msg=document.getElementById('dlMessage');const baseline=!v.org&&!v.dept&&!v.team&&!v.person&&!v.disease&&v.start===d.start&&v.end===d.end;msg.hidden=baseline;msg.textContent='当前仅提供 2026 年 7 月区域总览模拟数据。所选范围尚无数据，仍展示区域总览口径；点击重置可恢复默认查询。';},{orgs:{A机构:[],B机构:[],C机构:[]},people:[],diseases:d.outcomes.map(r=>r.name)},{start:d.start,end:d.end},{variant:'operations',orgLabel:'全部机构',applyOnChange:false});
