...
(() => {
  ...
  root.innerHTML=`<div class="dl-dashboard dv-dashboard">
    ...
    <div class="dv-funnel-risk-grid"><div class="dv-funnel-main"><p class="dv-overview-caption">本期筛查人群 · 向下转化</p><div id="dvCombinedFunnel" role="img" aria-label="总体人数12000，已筛查6000，筛查异常1200，符合入组1000，已入组800，当前在管720（本期入组人群），入组后管理率90%"></div><div class="dv-funnel-notes"><span>未筛查 ${f(t.due-t.screened)} 人</span><span>未见异常 ${f(t.screened-t.abnormal)} 人</span><span>不符合入组 ${d.ineligible} 人</span><span>未入组 ${t.eligible-t.enrolled} 人（超期 ${d.overdueEnrollment} 人）</span></div>${''}</div></div></section><aside class="dl-card dv-risk-panel"><h3>风险与安全</h3><p>统计截至 ${d.end} · 模拟数据</p><div class="dv-risk-kpis">${[['高危预警人数',d.riskTotals.high,'人'],['未处理预警人数',new Set(window.DashboardLatestData.records.alerts.filter(a=>a.status==='unhandled').map(a=>a.patientId)).size,'人'],['超时未处理任务',d.riskTotals.overdue,'项']].map(([name,n,unit])=>`<div><span>${name}</span><strong>${n}<small> ${unit}</small></strong></div>`).join('')}</div><div class="dv-risk-rate"><span>及时处理率</span><strong>${d.riskTotals.timely.toFixed(1)}%</strong><div class="dv-chart" data-value="${d.riskTotals.timely}" data-color="#3e8dff" data-compact="true"></div><small>及时处理 ${d.riskTotals.timelyCount} / 到期预警 ${d.riskTotals.due} 项</small></div><div class="dv-risk-rate"><span>预警闭环率</span><strong>${d.riskTotals.closed.toFixed(1)}%</strong><div class="dv-chart" data-value="${d.riskTotals.closed}" data-color="#21aa93" data-compact="true"></div><small>已闭环 ${d.riskTotals.closedCount} / 到期预警 ${d.riskTotals.due} 项</small></div><p class="dv-risk-explain">预警人数按患者去重；超时任务按预警明细统计。未处理预警共 ${d.riskTotals.unhandled} 项。</p></aside></div>
    ...
    <section class="dl-card dv-referrals">${title(4,'双向转诊')}<div class="dv-flows">${flow(false)}${flow(true)}</div>${table(['机构','上转申请（单）','上转未闭环（单）','上转闭环率','下转申请（单）','下转未闭环（单）','下转闭环率'],[referralTotal,...d.referrals].map(referralRow))}${note('按本期申请单跟踪；完成诊疗反馈或首次服务并确认结果后闭环。')}</section>
