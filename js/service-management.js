(function () {
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      .package-detail-hero { position: relative; width: 100%; aspect-ratio: 375 / 216; overflow: hidden; background: #eaf4ff; touch-action: pan-y; }
      .package-detail-hero-track { width: 100%; height: 100%; display: flex; transition: transform .32s ease; will-change: transform; }
      .package-detail-hero-slide { min-width: 100%; height: 100%; display: block; object-fit: cover; object-position: center; background: #e9eef6; }
      .package-detail-hero-slide.is-fallback { object-position: center top; }
      .package-detail-hero-nav { position: absolute; top: 50%; z-index: 2; width: 28px; height: 28px; padding: 0; display: grid; place-items: center; border: 0; border-radius: 50%; color: #50627d; background: rgba(255,255,255,.86); box-shadow: 0 4px 12px rgba(33,50,80,.16); font-size: 20px; line-height: 1; cursor: pointer; transform: translateY(-50%); }
      .package-detail-hero-nav.prev { left: 10px; }
      .package-detail-hero-nav.next { right: 10px; }
      .package-detail-hero-nav[hidden] { display: none; }
      .package-detail-hero-dots { position: absolute; left: 50%; bottom: 9px; z-index: 2; display: flex; gap: 5px; transform: translateX(-50%); }
      .package-detail-hero-dot { width: 6px; height: 6px; padding: 0; border: 0; border-radius: 999px; background: rgba(255,255,255,.64); box-shadow: 0 1px 4px rgba(21,41,73,.16); cursor: pointer; transition: width .2s, background .2s; }
      .package-detail-hero-dot.active { width: 18px; background: #fff; }
      ...
      .package-service-help { position: relative; display: inline-flex; vertical-align: middle; }
      .package-service-help button { display: inline-grid; place-items: center; width: 16px; height: 16px; padding: 0; border: 1px solid currentColor; border-radius: 50%; background: transparent; color: rgba(0,0,0,.45); font-size: 12px; cursor: pointer; }
      .package-service-help button:hover { color: #1677ff; }
      .package-service-help [role="note"] { position: absolute; z-index: 65; top: 26px; left: -100px; width: min(280px, 65vw); padding: 12px; border-radius: 8px; background: #fff; box-shadow: 0 6px 16px rgba(0,0,0,.08), 0 3px 6px -4px rgba(0,0,0,.12); color: rgba(0,0,0,.88); font-size: 14px; font-weight: 400; line-height: 1.6; }
      .package-field-error { color: #ff4d4f; font-size: 14px; margin-top: 4px; }
      .package-ant-select-selector[aria-invalid="true"] { border-color: #ff4d4f; }
      .package-ant-select.multiple .package-ant-select-selector { height: auto; min-height: 40px; padding: 4px 11px; }
      .package-ant-select.multiple .package-ant-select-value { display: flex; flex-wrap: wrap; gap: 4px; white-space: normal; }
      .package-select-tag { display: inline-flex; align-items: center; gap: 4px; max-width: 100%; padding: 0 7px; border-radius: 4px; background: rgba(0,0,0,.06); line-height: 24px; overflow-wrap: anywhere; }
      .package-select-tag [data-package-remove-option] { color: rgba(0,0,0,.45); }
      .package-ant-select.multiple .package-ant-select-option.selected::after { content: '✓'; margin-left: auto; color: #1677ff; }
      ...
      .package-detail-image-index.primary { background: #1677ff; }
  ...
  function injectViews() {
    ...
    document.body.insertAdjacentHTML('beforeend', `
      ...
      <section class="package-editor-overlay" id="packageEditorOverlay" data-note-scope="service-package-editor" data-note-mode="create" hidden aria-label="新建服务包">
        ...
        <div class="package-editor-scroll">
            ...
            <div class="package-phone">
              <div class="package-phone-screen">
                <div class="package-detail-preview">
                  <div class="package-detail-hero" id="packagePreviewHero" aria-label="详情头图轮播"><div class="package-detail-hero-track" id="packagePreviewHeroTrack"></div><button class="package-detail-hero-nav prev" data-package-hero-prev type="button" aria-label="上一张详情头图" hidden>‹</button><button class="package-detail-hero-nav next" data-package-hero-next type="button" aria-label="下一张详情头图" hidden>›</button><div class="package-detail-hero-dots" id="packagePreviewHeroDots" aria-label="详情头图分页"></div></div>
                  <main class="package-detail-main">
                    ...
                    <section class="package-detail-gallery" id="packagePreviewGalleryImages" aria-label="服务详情展示图" hidden></section>
          ...
          <form class="package-editor-card" id="packageEditorForm">
            <section class="package-config-section" data-config-section="basic"><div class="package-config-head"><span class="package-config-index">1</span><div><div class="package-config-title">基本信息</div><div class="package-config-subtitle">配置服务包在 C 端展示的基础内容</div></div></div><div class="package-field-grid">
              ...
              <div class="package-form-group full" data-package-detail-images><div class="package-detail-images-head"><label class="package-form-label"><span class="package-required">*</span>详情头图</label><span class="package-detail-images-count" id="packageDetailImagesCount">已上传 0/5</span></div><input id="packageDetailImagesInput" type="file" accept="image/jpeg,image/png,image/webp" multiple hidden><div class="package-detail-images-grid" id="packageDetailImagesGrid"></div><p class="package-detail-images-tip">第一张作为 C 端详情页默认首图；C 端顶部自动轮播并支持左右滑动。≡ 拖动图片可调整展示顺序；需上传 1～5 张，支持 JPG/JPEG/PNG/WebP，单张不超过 5MB，建议 750×420px（16:9）。</p></div>
              <div class="package-form-group full" data-package-gallery-images><div class="package-detail-images-head"><label class="package-form-label">详情展示图 <span class="package-optional">选填</span></label><span class="package-detail-images-count" id="packageGalleryImagesCount">已上传 0/9</span></div><input id="packageGalleryImagesInput" type="file" accept="image/jpeg,image/png,image/webp" multiple hidden><div class="package-detail-images-grid" id="packageGalleryImagesGrid"></div><p class="package-detail-images-tip">图片将在 C 端正文区域按配置顺序一张一张平铺展示。≡ 拖动图片可调整展示顺序；最多 9 张，支持 JPG/JPEG/PNG/WebP，单张不超过 5MB，建议 750×420px（16:9）。上传后自动压缩并保持原始比例。</p></div>
            ...
            <section class="package-config-section" data-config-section="plan"><div class="package-config-head"><span class="package-config-index">4</span><div><div class="package-config-title">服务配置 <span class="package-service-help"><button type="button" data-service-help aria-label="服务配置说明" aria-expanded="false" aria-controls="packageServiceHelp">?</button><span id="packageServiceHelp" role="note" hidden>配置服务包由哪些机构、科室、团队和人员提供，并可关联管理方案。</span></span></div><div class="package-config-subtitle">配置服务提供方及关联的管理方案</div></div></div><div class="package-field-grid">
              <div class="package-form-group"><label class="package-form-label" for="packageOrganizationInput"><span class="package-required">*</span>服务机构</label><select class="package-form-select" id="packageOrganizationInput" data-package-form-input data-placeholder="请选择服务机构（可多选）" multiple required aria-label="服务机构"><option value="健康管理中心">健康管理中心</option><option value="互联网医院">互联网医院</option><option value="第一人民医院">第一人民医院</option><option value="康复医学中心">康复医学中心</option><option value="市人民医院">市人民医院</option><option value="社区卫生服务中心">社区卫生服务中心</option></select><div id="packageOrganizationError" class="package-field-error" role="alert" hidden>请选择至少一个服务机构</div></div>
              <div class="package-form-group"><label class="package-form-label" for="packageDepartmentInput">服务科室 <span class="package-optional">选填</span></label><select class="package-form-select" id="packageDepartmentInput" data-package-form-input data-placeholder="请选择服务科室（可多选）" multiple aria-label="服务科室"><option value="健康管理科">健康管理科</option><option value="内分泌科">内分泌科</option><option value="心血管内科">心血管内科</option><option value="营养科">营养科</option><option value="康复医学科">康复医学科</option><option value="康复科">康复科</option></select></div>
              <div class="package-form-group"><label class="package-form-label" for="packageTeamInput">服务团队 <span class="package-optional">选填</span></label><select class="package-form-select" id="packageTeamInput" data-package-form-input data-placeholder="请选择服务团队（可多选）" multiple aria-label="服务团队"><option value="慢病健康管理团队">慢病健康管理团队</option><option value="营养与体重管理团队">营养与体重管理团队</option><option value="心血管专病服务团队">心血管专病服务团队</option><option value="术后康复随访团队">术后康复随访团队</option></select></div>
              <div class="package-form-group"><label class="package-form-label" for="packagePersonnelInput">服务人员 <span class="package-optional">选填</span></label><select class="package-form-select" id="packagePersonnelInput" data-package-form-input data-placeholder="请选择服务人员（可多选）" multiple aria-label="服务人员"><option value="李敏 医生">李敏 医生</option><option value="王悦 营养师">王悦 营养师</option><option value="周宁 健康管理师">周宁 健康管理师</option><option value="陈宇 康复师">陈宇 康复师</option><option value="王医生">王医生</option><option value="李医生">李医生</option><option value="张护士">张护士</option><option value="刘营养师">刘营养师</option></select></div>
              <div class="package-form-group full"><label class="package-form-label" for="packagePlanInput">关联方案 <span class="package-optional">选填</span></label><select class="package-form-select" id="packagePlanInput" data-package-form-input><option value="">请选择关联方案</option><option>90天健康减重管理方案</option><option>糖尿病全周期管理方案</option><option>高血压强化管理方案</option><option>心血管健康管理方案</option></select></div>
            </div></section>
        ...
        <div class="package-image-lightbox" id="packageDetailImageLightbox" hidden role="dialog" aria-modal="true" aria-label="查看服务包图片"><button class="package-image-lightbox-close" data-close-package-detail-image type="button" aria-label="关闭大图">×</button><img id="packageDetailImageLightboxPreview" alt="服务包图片大图预览"></div>
      </section>
    `);
    return true;
  }
  ...
  function syncPackageAntSelect(select) {
    ...
    wrapper.classList.toggle('placeholder', select.multiple ? !select.selectedOptions.length : /^请选择/.test(selected?.textContent || ''));
    wrapper.classList.toggle('multiple', select.multiple);
    if (value) {
      value.replaceChildren();
      if (!select.multiple) value.textContent = selected?.textContent || '';
      else {
        if (!select.selectedOptions.length) value.textContent = select.dataset.placeholder;
        Array.from(select.selectedOptions).forEach(option => {
          const tag = document.createElement('span');
          tag.className = 'package-select-tag';
          tag.textContent = option.textContent;
          const remove = document.createElement('span');
          remove.textContent = '×';
          remove.dataset.packageRemoveOption = String(option.index);
    ...
    wrapper.querySelectorAll('[data-package-ant-option]').forEach(option => {
      const isSelected = select.options[Number(option.dataset.packageAntOption)]?.selected;
  ...
  function enhancePackageSelects() {
    document.querySelectorAll('#packageEditorOverlay select').forEach(select => {
      ...
      wrapper.className = 'package-ant-select' + (select.multiple ? ' multiple' : '');
  ...
  function setPackageCover(source = '', fileName = '') {
    ...
    if (!overlay || !thumb || !empty || !replace || !remove || !meta) return;
  ...
  const packageDetailImageLimit = 5;
  let packageDetailImages = [];
  let packageDetailReplaceId = '';
  let sortingPackageDetailImageId = '';
  const packageGalleryImageLimit = 9;
  let packageGalleryImages = [];
  let packageGalleryReplaceId = '';
  let sortingPackageGalleryImageId = '';
  let packageHeroIndex = 0;
  let packageHeroTimer = 0;
  let packageHeroTouchStartX = null;
  ...
  function updatePackageDetailImagesPreview() {
    const track = document.getElementById('packagePreviewHeroTrack');
    const dots = document.getElementById('packagePreviewHeroDots');
    const previous = document.querySelector('[data-package-hero-prev]');
    const next = document.querySelector('[data-package-hero-next]');
    if (!track || !dots) return;
    window.clearInterval(packageHeroTimer);
    const items = packageDetailImages.length ? packageDetailImages : [{ source: defaultPackageCover, name: '默认详情头图', fallback: true }];
    packageHeroIndex = Math.min(packageHeroIndex, items.length - 1);
    track.replaceChildren();
    dots.replaceChildren();
    items.forEach((item, index) => {
      const image = document.createElement('img');
      image.className = `package-detail-hero-slide${item.fallback ? ' is-fallback' : ''}`;
      image.src = item.source;
      image.alt = `详情头图 ${String(index + 1).padStart(2, '0')}`;
      image.addEventListener('error', () => {
        if (image.dataset.fallbackApplied) return;
        image.dataset.fallbackApplied = 'true';
        image.classList.add('is-fallback');
        image.src = defaultPackageCover;
      });
      track.appendChild(image);
      if (items.length > 1) {
        const dot = document.createElement('button');
        dot.className = 'package-detail-hero-dot';
        dot.type = 'button';
        dot.dataset.packageHeroIndex = String(index);
        dot.setAttribute('aria-label', `查看第 ${index + 1} 张详情头图`);
        dots.appendChild(dot);
      }
    });
    if (previous) previous.hidden = items.length < 2;
    if (next) next.hidden = items.length < 2;
    setPackageHeroIndex(packageHeroIndex, false);
    if (items.length > 1) packageHeroTimer = window.setInterval(() => setPackageHeroIndex(packageHeroIndex + 1, false), 4000);
  }

  function setPackageHeroIndex(index, restartTimer = true) {
    const track = document.getElementById('packagePreviewHeroTrack');
    const slides = track ? Array.from(track.children) : [];
    if (!track || !slides.length) return;
    packageHeroIndex = (Number(index) + slides.length) % slides.length;
    track.style.transform = `translateX(-${packageHeroIndex * 100}%)`;
    document.querySelectorAll('[data-package-hero-index]').forEach((dot, dotIndex) => {
      dot.classList.toggle('active', dotIndex === packageHeroIndex);
      dot.setAttribute('aria-current', dotIndex === packageHeroIndex ? 'true' : 'false');
    });
    if (restartTimer && slides.length > 1) {
      window.clearInterval(packageHeroTimer);
      packageHeroTimer = window.setInterval(() => setPackageHeroIndex(packageHeroIndex + 1, false), 4000);
    }
  }
  ...
  function renderPackageDetailImages() {
    ...
    packageDetailImages.forEach((item, index) => {
      ...
      card.draggable = true;
      card.tabIndex = 0;
      card.setAttribute('aria-label', `详情头图 ${index + 1}，可拖动排序`);

      const sequence = document.createElement('span');
      sequence.className = 'package-detail-image-index';
      sequence.textContent = index === 0 ? '01 · 首图' : String(index + 1).padStart(2, '0');
      sequence.classList.toggle('primary', index === 0);
      ...
      previewButton.setAttribute('aria-label', `查看详情头图 ${index + 1} 大图`);
      const image = document.createElement('img');
      image.src = item.source;
      image.alt = `详情头图 ${index + 1}`;
      image.addEventListener('error', () => {
        if (image.dataset.fallbackApplied) return;
        image.dataset.fallbackApplied = 'true';
        image.src = defaultPackageCover;
  ...
  function setPackageDetailImages(items = []) {
    packageDetailImages = Array.isArray(items) ? items.slice(0, packageDetailImageLimit).map(item => ({
      ...
      name: typeof item === 'string' ? '详情头图' : item?.name || '详情头图'
  ...
  async function handlePackageDetailImageFiles(fileList) {
    ...
    if (input) input.value = '';
    renderPackageDetailImages();
    if (successCount) showPackageToast(`${isReplacing ? '图片已替换' : `已上传 ${successCount} 张详情头图`}，C端预览已同步`);
  }

  function openPackageDetailImageLightbox(id) {
    const item = packageDetailImages.find(image => image.id === id) || packageGalleryImages.find(image => image.id === id);
    ...
    preview.onerror = () => {
      ...
      preview.src = defaultPackageCover;
      preview.alt = '图片加载失败，已显示默认占位图';
    };
    preview.dataset.fallbackApplied = 'false';
    preview.src = item.source;
    preview.alt = item.name || '详情头图大图预览';
  ...
  function updatePackageGalleryImagesPreview() {
    const preview = document.getElementById('packagePreviewGalleryImages');
    if (!preview) return;
    preview.replaceChildren();
    packageGalleryImages.forEach((item, index) => {
      const image = document.createElement('img');
      image.src = item.source;
    ...
    preview.hidden = packageGalleryImages.length === 0;
  }

  function renderPackageGalleryImages() {
    const grid = document.getElementById('packageGalleryImagesGrid');
    const count = document.getElementById('packageGalleryImagesCount');
    if (!grid || !count) return;
    grid.replaceChildren();
    packageGalleryImages.forEach((item, index) => {
      const card = document.createElement('article');
      card.className = 'package-detail-image-card';
      card.dataset.packageGalleryImageId = item.id;
      card.draggable = true;
      card.tabIndex = 0;
      ...
      previewButton.className = 'package-detail-image-preview';
      previewButton.type = 'button';
      previewButton.dataset.viewPackageGalleryImage = item.id;
      ...
      const actions = document.createElement('div');
      actions.className = 'package-detail-image-actions';
      [['查看', 'viewPackageGalleryImage'], ['替换', 'replacePackageGalleryImage'], ['删除', 'deletePackageGalleryImage']].forEach(([label, dataKey]) => {
        const button = document.createElement('button');
        button.type = 'button';
        button.textContent = label;
        button.dataset[dataKey] = item.id;
    ...
    if (packageGalleryImages.length < packageGalleryImageLimit) {
      const add = document.createElement('button');
      add.className = 'package-detail-image-add';
      add.type = 'button';
      add.dataset.addPackageGalleryImage = '';
      add.innerHTML = '<strong>＋</strong><span>上传图片</span>';
      grid.appendChild(add);
    }
    count.textContent = `已上传 ${packageGalleryImages.length}/${packageGalleryImageLimit}`;
    updatePackageGalleryImagesPreview();
  }

  function setPackageGalleryImages(items = []) {
    packageGalleryImages = Array.isArray(items) ? items.slice(0, packageGalleryImageLimit).map(item => ({
      id: item?.id || createPackageDetailImageId(),
      source: typeof item === 'string' ? item : item?.source || '',
      name: typeof item === 'string' ? '详情展示图' : item?.name || '详情展示图'
    })).filter(item => item.source) : [];
    packageGalleryReplaceId = '';
    renderPackageGalleryImages();
  }

  async function handlePackageGalleryImageFiles(fileList) {
    const files = Array.from(fileList || []);
    if (!files.length) return;
    const input = document.getElementById('packageGalleryImagesInput');
    const isReplacing = Boolean(packageGalleryReplaceId);
    if (packageGalleryReplaceId) files.splice(1);
    else files.splice(Math.max(0, packageGalleryImageLimit - packageGalleryImages.length));
    if (!packageGalleryReplaceId && !files.length) {
    ...
    for (const file of files) {
      ...
      try {
        const source = await compressPackageDetailImage(file);
        const imageItem = { id: createPackageDetailImageId(), source, name: file.name };
        if (packageGalleryReplaceId) {
          const index = packageGalleryImages.findIndex(item => item.id === packageGalleryReplaceId);
          if (index >= 0) packageGalleryImages.splice(index, 1, { ...imageItem, id: packageGalleryReplaceId });
          packageGalleryReplaceId = '';
        } else if (packageGalleryImages.length < packageGalleryImageLimit) packageGalleryImages.push(imageItem);
        successCount += 1;
      } catch (error) {
        showPackageToast(`${file.name} 处理失败，请重新上传`);
      }
  ...
  function syncPackageGalleryImagesFromDom() {
    const imageMap = new Map(packageGalleryImages.map(item => [item.id, item]));
    const ids = Array.from(document.querySelectorAll('[data-package-gallery-image-id]')).map(card => card.dataset.packageGalleryImageId);
    packageGalleryImages = ids.map(id => imageMap.get(id)).filter(Boolean);
    updatePackageGalleryImagesPreview();
  }

  function handlePackageGalleryImageDragStart(event) {
    const card = event.target.closest('[data-package-gallery-image-id]');
    if (!card || event.target.closest('.package-detail-image-actions button')) return;
    sortingPackageGalleryImageId = card.dataset.packageGalleryImageId;
    card.classList.add('sorting');
    event.dataTransfer.effectAllowed = 'move';
    event.dataTransfer.setData('text/plain', sortingPackageGalleryImageId);
  }

  function handlePackageGalleryImageDragOver(event) {
    if (!sortingPackageGalleryImageId) return;
    const target = event.target.closest('[data-package-gallery-image-id]');
    const source = document.querySelector(`[data-package-gallery-image-id="${sortingPackageGalleryImageId}"]`);
    if (!target || !source || target === source) return;
    event.preventDefault();
    event.dataTransfer.dropEffect = 'move';
    document.querySelectorAll('[data-package-gallery-image-id].sort-target').forEach(card => card.classList.remove('sort-target'));
    target.classList.add('sort-target');
    const rect = target.getBoundingClientRect();
    const sourceRect = source.getBoundingClientRect();
    const sameRow = Math.abs(sourceRect.top - rect.top) < rect.height / 2;
    const insertAfter = sameRow ? event.clientX > rect.left + rect.width / 2 : event.clientY > rect.top + rect.height / 2;
    target.parentElement.insertBefore(source, insertAfter ? target.nextElementSibling : target);
    syncPackageGalleryImagesFromDom();
  }

  function finishPackageGalleryImageSort(event) {
    if (!sortingPackageGalleryImageId) return;
    event?.preventDefault?.();
    syncPackageGalleryImagesFromDom();
    document.querySelectorAll('[data-package-gallery-image-id].sorting,[data-package-gallery-image-id].sort-target').forEach(card => card.classList.remove('sorting', 'sort-target'));
    sortingPackageGalleryImageId = '';
    renderPackageGalleryImages();
  }

  function movePackageGalleryImageByKeyboard(card, direction) {
    const current = packageGalleryImages.findIndex(item => item.id === card.dataset.packageGalleryImageId);
    const target = current + direction;
    if (current < 0 || target < 0 || target >= packageGalleryImages.length) return;
    [packageGalleryImages[current], packageGalleryImages[target]] = [packageGalleryImages[target], packageGalleryImages[current]];
    const movedId = packageGalleryImages[target].id;
    renderPackageGalleryImages();
    document.querySelector(`[data-package-gallery-image-id="${movedId}"]`)?.focus();
  }
  ...
  function openPackageEditor(item = null) {
    ...
    const savedGalleryImages = item?.[13] || [];
    const savedHeaderImages = item?.[14] || [];
    setPackageDetailImages(item && !savedHeaderImages.length ? [{ source: item?.[9] || defaultPackageCover, name: '默认详情头图' }] : savedHeaderImages);
    setPackageGalleryImages(savedGalleryImages);
  ...
  function setPackageOrganizationError(invalid) {
    const select = document.getElementById('packageOrganizationInput');
    const trigger = select.closest('.package-ant-select').querySelector('[data-package-ant-select-trigger]');
    trigger.setAttribute('aria-required', 'true');
    trigger.setAttribute('aria-invalid', String(invalid));
    trigger.setAttribute('aria-describedby', 'packageOrganizationError');
    document.getElementById('packageOrganizationError').hidden = !invalid;
  }

  function savePackageDraft() {
    const organization = document.getElementById('packageOrganizationInput');
    if (!organization.selectedOptions.length) {
      setPackageOrganizationError(true);
      organization.closest('.package-form-group').scrollIntoView({ behavior: 'smooth', block: 'center' });
      organization.closest('.package-ant-select').querySelector('button').focus();
      return;
    }
    const overlay = document.getElementById('packageEditorOverlay');
    const code = overlay?.dataset.editCode;
    if (!packageDetailImages.length) {
      showPackageToast('请至少上传 1 张详情头图');
      const imageSection = document.querySelector('[data-package-detail-images]');
  ...
  document.addEventListener('dragstart', handlePackageGalleryImageDragStart);
  document.addEventListener('dragover', handlePackageGalleryImageDragOver);
  document.addEventListener('drop', finishPackageGalleryImageSort);
  document.addEventListener('dragend', finishPackageGalleryImageSort);
  document.addEventListener('touchstart', event => {
    if (!event.target.closest('#packagePreviewHero')) return;
    packageHeroTouchStartX = event.touches?.[0]?.clientX ?? null;
  }, { passive: true });
  document.addEventListener('touchend', event => {
    if (packageHeroTouchStartX === null || !event.target.closest('#packagePreviewHero')) return;
    const endX = event.changedTouches?.[0]?.clientX;
    const distance = typeof endX === 'number' ? endX - packageHeroTouchStartX : 0;
    packageHeroTouchStartX = null;
    if (Math.abs(distance) < 36) return;
    setPackageHeroIndex(packageHeroIndex + (distance < 0 ? 1 : -1));
  }, { passive: true });

  document.addEventListener('click', event => {
    const help = document.querySelector('[data-service-help]');
    const content = document.getElementById('packageServiceHelp');
    if (event.target.closest('[data-service-help]')) {
      content.hidden = !content.hidden;
      help.setAttribute('aria-expanded', String(!content.hidden));
      return;
    }
    if (!event.target.closest('.package-service-help')) {
      content.hidden = true;
      help.setAttribute('aria-expanded', 'false');
    }
    const remove = event.target.closest('[data-package-remove-option]');
    if (remove) {
      const select = remove.closest('.package-ant-select').querySelector('select');
      if (select.disabled) return;
      select.options[Number(remove.dataset.packageRemoveOption)].selected = false;
      syncPackageAntSelect(select);
    ...
    if (packageSelectOption) {
      ...
      const option = select.options[Number(packageSelectOption.dataset.packageAntOption)];
    ...
    if (event.target.closest('[data-add-package-gallery-image]')) {
      packageGalleryReplaceId = '';
      const input = document.getElementById('packageGalleryImagesInput');
      if (input) {
        input.multiple = true;
    ...
    if (event.target.closest('[data-package-hero-prev]')) {
      setPackageHeroIndex(packageHeroIndex - 1);
      return;
    }
    if (event.target.closest('[data-package-hero-next]')) {
      setPackageHeroIndex(packageHeroIndex + 1);
      return;
    }
    const heroDot = event.target.closest('[data-package-hero-index]');
    if (heroDot) {
      setPackageHeroIndex(Number(heroDot.dataset.packageHeroIndex));
      return;
    }
    ...
    const viewGalleryImage = event.target.closest('[data-view-package-gallery-image]');
    if (viewGalleryImage) {
      openPackageDetailImageLightbox(viewGalleryImage.dataset.viewPackageGalleryImage);
      return;
    }
    ...
    const replaceGalleryImage = event.target.closest('[data-replace-package-gallery-image]');
    if (replaceGalleryImage) {
      packageGalleryReplaceId = replaceGalleryImage.dataset.replacePackageGalleryImage;
      const input = document.getElementById('packageGalleryImagesInput');
      if (input) {
        input.multiple = false;
        input.click();
    ...
    if (deleteDetailImage) {
      const editItem = getPackageData(document.getElementById('packageEditorOverlay')?.dataset.editCode || '');
      if (packageDetailImages.length <= 1 && editItem?.[8] === '已上架') {
        showPackageToast('已上架服务包至少保留 1 张详情头图');
        return;
      }
      packageDetailImages = packageDetailImages.filter(item => item.id !== deleteDetailImage.dataset.deletePackageDetailImage);
      packageHeroIndex = 0;
      renderPackageDetailImages();
      showPackageToast('详情头图已删除，其余图片已自动补位');
      return;
    }
    const deleteGalleryImage = event.target.closest('[data-delete-package-gallery-image]');
    if (deleteGalleryImage) {
      packageGalleryImages = packageGalleryImages.filter(item => item.id !== deleteGalleryImage.dataset.deletePackageGalleryImage);
      renderPackageGalleryImages();
  ...
  document.addEventListener('change', event => {
    if (event.target.id === 'packageOrganizationInput') setPackageOrganizationError(!event.target.selectedOptions.length);
    ...
    if (event.target.id === 'packageGalleryImagesInput') handlePackageGalleryImageFiles(event.target.files);
  ...
  document.addEventListener('keydown', event => {
    ...
    const galleryImageCard = event.target.closest('[data-package-gallery-image-id]');
    if (galleryImageCard && ['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(event.key) && (event.altKey || event.ctrlKey)) {
      event.preventDefault();
      movePackageGalleryImageByKeyboard(galleryImageCard, ['ArrowLeft', 'ArrowUp'].includes(event.key) ? -1 : 1);
      return;
    }
